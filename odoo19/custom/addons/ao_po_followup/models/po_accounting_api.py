...
class PoAccountingApi(models.AbstractModel):
    _name = 'po.accounting.api'
    _description = 'Accounting Odoo API Client'

    @api.model
    def _get_settings(self):
        ICP = self.env['ir.config_parameter'].sudo()
        url = (ICP.get_param('ao_po_followup.accounting_url') or '').rstrip('/')
        login = ICP.get_param('ao_po_followup.accounting_login') or ''
        password = ICP.get_param('ao_po_followup.accounting_password') or ''
        if not url or not login or not password:
            raise UserError(_(
                'Accounting API is not configured. '
                'Set URL, login and password under PO Follow-up > Configuration > Settings.'
            ))
    ...
    def create_draft_invoices(self, items):
        ...
        endpoint = f'{url}/api/unified_contract/invoices'
