...
class PoOrderApiController(http.Controller):
    ...
    def _authenticate_basic(self):
        ...
        expected_login = ICP.get_param('ao_po_followup.po_order_api_login') or ''
        expected_password = ICP.get_param('ao_po_followup.po_order_api_password') or ''
        ...
        try:
            ...
            _logger.exception('PO Order API login fallback failed')
            uid = False
        if not uid:
            return False, request.make_json_response({
                'status': 'error',
    ...
    def receive_po_order(self, **kwargs):
        try:
            ...
            results = []
            PoOrder = request.env['po.order'].sudo()
            for item in items:
                try:
                    ...
                    _logger.exception('Failed to receive PO Order item')
                    results.append({
                        'ok': False,
                        'po_number': item.get('po_number') or item.get('name'),
                        'error': str(exc),
            ...
            _logger.exception('PO Order receive API crashed')
            return request.make_json_response({
                'status': 'error',
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    _logger.exception('Failed to archive PO Order item')
                    results.append({
                        'ok': False,
                        'po_number': item.get('po_number') or item.get('name'),
                        'error': str(exc),
            ...
            _logger.exception('PO Order archive API crashed')
            return request.make_json_response({
                'status': 'error',
