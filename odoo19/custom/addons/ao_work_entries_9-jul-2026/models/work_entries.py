...
class HrEmployeeAttendanceGrid(models.Model):
    ...
    def _format_attendance_day_cell(self, day_data):
        ...
        Yellow warning when check-in is after 08:30 or check-out is before 16:30.
        """
        status = day_data.get('status', 'absent')
        if status == 'present':
            ...
            late_in = bool(
                check_in_dt and check_in_dt.time() > time(8, 30)
            )
            early_out = bool(
    ...
    @api.model
    def generate_attendance_summary_excel(self, date_from, date_to, department_id=False, print_all=False):
        ...
        try:
            ...
            domain = [('active', '=', True)]
            if not print_all and department_id:
            ...
            department_name = self.env['hr.department'].browse(department_id).name if department_id else False
    ...
    def _get_payslip_absence_days(self, employee_id, year, month, dept_id, dept_summary, dept_data):
        """Resolve absence days for payslip creation, honoring manual monthly overrides."""
        manual_absence_days = self._get_manual_absence_days(employee_id, year, month)
        if manual_absence_days is None:
            return dept_data.get('absence_days', 0)
        ...
        if main_dept_id and main_dept_id in dept_summary:
            target_dept_id = main_dept_id
        elif dept_summary:
            target_dept_id = next(iter(dept_summary.keys()))
        else:
            return manual_absence_days

        return manual_absence_days if dept_id == target_dept_id else 0
                ...
                try:
                    ...
                    if not dept_summary_result.get('success') or not dept_summary_result.get('department_summary'):
                        ...
                        try:
                            ...
                            manual_absence_days = self._get_manual_absence_days(
                                employee['id'], year, month
                            )

                            # Add custom fields with zero values
                            if hasattr(Payslip, 'x_number_of_absence_days'):
                                payslip_vals['x_number_of_absence_days'] = manual_absence_days or 0
                            ...
                            created_payslips.append({
                                ...
                                'absence_days': manual_absence_days or 0,
                                'late_hours': 0.0,
                            })
                    ...
                    for dept_id, dept_data in dept_summary.items():
                        try:
                            ...
                            absence_days = self._get_payslip_absence_days(
                                employee['id'], year, month, dept_id, dept_summary, dept_data
                            )

                            # Add custom fields if they exist
                            if hasattr(Payslip, 'x_number_of_absence_days'):
                                payslip_vals['x_number_of_absence_days'] = absence_days
    ...
    def _post_remaining_leave_days_to_payslip(self, employee_id, year, month, days):
        """Post Arabic leave-days message to the employee payslip chatter."""
        if 'hr.payslip' not in self.env:
            return
    ...
    @api.model
    def update_employee_monthly_summary(self, employee_id, year, month, field_name, value, mark_absence_manual=False):
        """
        ...
        try:
            ...
            # Validate value
            if field_name in ('total_absence_days', 'remaining_leave_days'):
