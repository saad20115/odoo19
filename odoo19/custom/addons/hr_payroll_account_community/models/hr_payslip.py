...
class HrPayslip(models.Model):
    ...
    def action_payslip_done(self):
        """Finalize and post the payroll slip, creating accounting entries.

        When debit/credit accounts are missing on salary rules or the journal
        has no default account for balancing, skip journal-entry creation
        instead of raising — the payslip is still confirmed as Done.
        """
        ...
        for slip in self:
            ...
            debit_sum = 0.0
            credit_sum = 0.0
            name = 'Payslip of ' + slip.employee_id.name
            move_dict = {
                'narration': name,
                'ref': slip.number,
            ...
            # Skip accounting entry when nothing to post (no rule accounts).
            if not line_ids:
                continue
            ...
            if slip.company_id.currency_id.compare_amounts(
                ...
                if not acc_id:
                    # Unbalanced and no journal default account → skip JE
                    skip_move = True
                else:
                    adjust_credit = (0, 0, {
            ...
            if not move.line_ids:
                # Accounts incomplete — leave payslip Done without posting JE
                move.unlink()
                slip.write({'move_id': False})
                continue
            move.action_post()
        return res
