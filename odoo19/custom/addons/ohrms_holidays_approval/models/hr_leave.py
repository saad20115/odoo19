...
class HrLeave(models.Model):
    ...
    full_admin_approver_id = fields.Many2one(
        'res.users',
        string='Full Admin Approver',
        readonly=True,
    )

    def action_full_admin_approve(self):
        self.ensure_one()
        current_uid = self.env.uid

        # Check permission
        if current_uid not in self.holiday_status_id.full_admin_ids.ids:
            raise UserError("You are not allowed to fully approve this leave.")

        # Approve directly
        self.write({
            'state': 'validate',
            'full_admin_approver_id': current_uid,
        })

        # Chatter log
        self.message_post(
            body="Leave fully approved by Admin Override: %s" % self.env.user.name
        )
