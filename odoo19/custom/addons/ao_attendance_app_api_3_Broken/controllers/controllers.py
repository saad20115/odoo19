...
class AppLogin(http.Controller, MultiCompanyEmployeeMixin):
    ...
    def select_companies(self, **kwargs):
        try:
            ...
            Employee = request.env["hr.employee"].sudo()
            emp_companies = Employee.search([
                ("user_id", "=", u.id),
                ("company_id", "!=", False),
            ])
            allowed_ids = list(dict.fromkeys(emp_companies.mapped("company_id").ids))

            if not allowed_ids:
                return invalid_response("no_employee_companies", "User has no employee in any company", 403)
    ...
    def check_in_out(self, **post):
        try:
            ...
            if not employee:
                return invalid_response("no_employee", "No employee linked to this user in the selected company", 404)
    ...
    def get_employee_new_expenses(self, **post):
        ...
        try:
            ...
            if not allowed_company_ids:
                return invalid_response("no_companies", "No companies selected", 400)
...
class TimeoffController(http.Controller, MultiCompanyEmployeeMixin):
    ...
    def create_timeoff_request(self, **kwargs):
        try:
            ...
            payload = request.httprequest.data.decode() or "{}"
            payload = json.loads(payload)
            ...
            if not leave_type.exists():
                return invalid_response("not_found", "Time off type not found", 404)

            allowed_ids = envc.context.get("allowed_company_ids") or [current_company_id]
            allowed_ids = [int(x) for x in allowed_ids if x]
            ...
            ctx.update({
                ...
                "force_company": int(current_company_id),
            })
            ...
            if number_of_days is not None:
                try:
                    ...
                    return invalid_response("invalid_days", "number_of_days must be a number", 400)
            else:
                requested_days = None

            # --- Negative balance / Allow override (حسب إعداد نوع الإجازة) ---
            allow_negative = bool(
            ...
            if leave_type.requires_allocation and not allow_negative:
                if requested_days is None:
                    requested_days = 1.0

                if float(remaining) < float(requested_days):
                    return invalid_response(
                        "no_allocation",
                        f"You do not have any allocation for this time off type. Remaining: {float(remaining)}",
                        400
                    )
            ...
            timeoff_request = envc['hr.leave'].sudo().create(vals)

            timeoff_request.validation_status_ids = [(5, 0, 0)]
            existing_validator_user_ids = timeoff_request.validation_status_ids.mapped("user_id").ids

            li = []
            for v in timeoff_request.holiday_status_id.validator_ids.filtered(
                    lambda x: x.user_id.id not in existing_validator_user_ids
            ):
                li.append((0, 0, {"user_id": v.user_id.id}))
            timeoff_request.validation_status_ids = li
            ...
            response_data = {
                ...
                "request_date_from": timeoff_request.request_date_from.strftime(
                    '%Y-%m-%d') if timeoff_request.request_date_from else None,
                "request_date_to": timeoff_request.request_date_to.strftime(
                    '%Y-%m-%d') if timeoff_request.request_date_to else None,
                ...
                "remaining_before_submit": float(remaining) if remaining is not None else None,
            }
    ...
    def get_timeoff_requests(self, **kwargs):
        """Get time-off requests from all selected companies"""
        try:
            user = request.env.user
            allowed_company_ids = self._allowed_company_ids()
            ...
            for company_id, envc in self._iter_company_envs():
                ...
                timeoff_requests = envc['hr.leave'].sudo().search(domain, order='create_date desc')
                ...
                for req in timeoff_requests:
                    request_data = {
                        ...
                        "request_date_from": req.request_date_from.strftime(
                            '%Y-%m-%d') if req.request_date_from else None,
                    ...
                    if len(allowed_company_ids) > 1:
                        ...
                        request_data["company_name"] = req.employee_id.company_id.name
            ...
            _logger.error(f"Error in get_timeoff_requests: {str(e)}", exc_info=True)
            return valid_response({"error": str(e)}, status=500)
    ...
    def get_pending_requests(self, **kwargs):
        """Get pending time-off requests from all selected companies"""
        try:
            user = request.env.user
            allowed_company_ids = self._allowed_company_ids()
            ...
            for company_id, envc in self._iter_company_envs():
                ...
                pending_requests = envc['hr.leave'].sudo().search(domain, order='create_date desc')
            ...
            _logger.error(f"Error in get_pending_requests: {str(e)}", exc_info=True)
            return valid_response({"error": str(e)}, status=500)
    ...
    def get_all_pending_requests(self, **kwargs):
        try:
            ...
            domain = [
                ...
                '|',
                ('holiday_status_id.full_admin_ids', 'in', [user_id]),
                '&',
                ('validation_status_ids.user_id', '=', user_id),
                ('validation_status_ids.validation_status', '=', False)
            ]

            total_count = envc['hr.leave'].sudo().search_count(domain)

            pending_requests = envc['hr.leave'].sudo().search(
                domain,
            ...
            requests_data = []
            for req in pending_requests:
                is_full_admin = user_id in req.holiday_status_id.full_admin_ids.ids
                is_validator = user_id in req.validation_status_ids.filtered(
                    lambda v: not v.validation_status
                ).mapped('user_id').ids

                user_validation_status = req.validation_status_ids.filtered(
                    lambda v: v.user_id.id == user_id
                )
                ...
                requests_data.append({
                    ...
                    "state_display": dict(req._fields['state'].selection)[req.state],
                    "create_date": req.create_date.strftime('%Y-%m-%d %H:%M:%S') if req.create_date else None,
                    "can_full_approve": is_full_admin,
                    "is_validator": is_validator,
                    "multi_level_validation": req.multi_level_validation,
                    "my_approval_status": user_validation_status[
                        0].validation_status if user_validation_status else None,
                    "validators_approved": len(req.validation_status_ids.filtered(lambda v: v.validation_status)),
                    "validators_total": len(req.validation_status_ids)
                })
    ...
    def get_approved_requests(self, **kwargs):
        """Get approved time-off requests from all selected companies"""
        try:
            user = request.env.user
            allowed_company_ids = self._allowed_company_ids()
            ...
            for company_id, envc in self._iter_company_envs():
                ...
                approved_requests = envc['hr.leave'].sudo().search([
                    ('employee_id', '=', employee.id),
                    ('employee_id.company_id', '=', company_id),
                    ('state', 'in', ['validate'])
            ...
            _logger.error(f"Error in get_approved_requests: {str(e)}", exc_info=True)
            return valid_response({"error": str(e)}, status=500)
    ...
    def get_refused_requests(self, **kwargs):
        """Get refused time-off requests from all selected companies"""
        try:
            user = request.env.user
            allowed_company_ids = self._allowed_company_ids()
            ...
            for company_id, envc in self._iter_company_envs():
                ...
                refused_requests = envc['hr.leave'].sudo().search([
                    ('employee_id', '=', employee.id),
                    ('employee_id.company_id', '=', company_id),
                    ('state', '=', 'refuse')
                ], order='create_date desc')
            ...
            _logger.error(f"Error in get_refused_requests: {str(e)}", exc_info=True)
            return valid_response({"error": str(e)}, status=500)
    ...
    def get_timeoff_balance(self, **kwargs):
        """Get time-off balance from all selected companies"""
        try:
            user = request.env.user
            allowed_company_ids = self._allowed_company_ids()
            ...
            holiday_status_id = kwargs.get("holiday_status_id")  # optional
            ...
            # Track leave types across companies to avoid duplicates
            seen_leave_types = {}
            ...
            for company_id, envc in self._iter_company_envs():
                ...
                for holiday_type in holiday_types:
                    ...
                    # Get approved leaves
                    approved_leaves = envc['hr.leave'].sudo().search([
                        ('employee_id', '=', employee.id),
                        ('employee_id.company_id', '=', company_id),
                        ('holiday_status_id', '=', holiday_type.id),
                        ('state', '=', 'validate'),
                    ...
                    total_spent = sum(approved_leaves.mapped('number_of_days')) or 0.0
                    remaining_days = float(total_allocated) - float(total_spent)

                    # Create unique key for leave type
                    type_key = f"{holiday_type.id}_{company_id}" if len(allowed_company_ids) > 1 else str(
                        holiday_type.id)

                    if type_key not in seen_leave_types:
                        balance_item = {
                        ...
                        if len(allowed_company_ids) > 1:
                            balance_item["company_id"] = company_id
                            balance_item["company_name"] = envc['res.company'].sudo().browse(company_id).name

                        all_types_balance.append(balance_item)
                        seen_leave_types[type_key] = True

                        grand_total_allocated += total_allocated
                        grand_total_spent += total_spent
                        grand_total_remaining += remaining_days

            return valid_response({
                'types': all_types_balance,
                'all_allocated': grand_total_allocated,
                'all_spent': grand_total_spent,
                'all_remaining': grand_total_remaining,
            })

        except Exception as e:
            _logger.error(f"Error in get_timeoff_balance: {str(e)}", exc_info=True)
            return valid_response({"error": str(e)}, status=500)
    ...
    def cancel_timeoff_request(self, **kwargs):
        """Cancel a time-off request (only if it's in draft or confirm state)
           Search across selected companies (allowed_company_ids from the company selector)
        """
        try:
            payload = request.httprequest.data.decode() or "{}"
            payload = json.loads(payload)
            ...
            if not request_id:
                return valid_response({"error": "Request ID is required"}, status=400)

            allowed_company_ids = self._allowed_company_ids() or []
            if not allowed_company_ids:
                allowed_company_ids = [int(self._current_company_id())]

            req_id = int(request_id)
            user = request.env.user

            timeoff_request = request.env['hr.leave']
            found_employee = None
            found_company_id = None

            for company_id, envc in self._iter_company_envs():
                if int(company_id) not in [int(x) for x in allowed_company_ids]:
                    continue
                ...
                if leave:
                    timeoff_request = leave
                    found_employee = employee
                    found_company_id = company_id
                    break

            if not timeoff_request or not timeoff_request.exists():
                return valid_response({"error": "Time-off request not found"}, status=404)

            if not found_employee:
                return invalid_response("no_employee", "No employee linked to this user in the selected companies", 404)
            ...
            if hasattr(timeoff_request, "action_refuse"):
                timeoff_request.sudo().action_refuse()
            elif hasattr(timeoff_request, "action_cancel"):
                timeoff_request.sudo().action_cancel()
            else:
                if hasattr(timeoff_request, "action_draft"):
