...
class AttendanceGridComponent extends Component {
  ...
  async loadData() {
    ...
    try {
      ...
      if (result.success) {
        ...
        this.state.monthlySummaries = result.monthly_summaries || {}; // NEW
        await this.loadDepartmentSummaries();
      ...
      this.state.stats = {};
      this.state.monthlySummaries = {}; // NEW
    } finally {
  ...
  getAttendanceCellClass(employeeId, day) {
    ...
    return `o_attendance_cell ${statusClass} ${leaveIndicator}`.trim();
  }
  ...
  // ADD this new method to calculate all metrics
  ...
  getSummaryValue(employeeId, fieldName) {
    ...
    if (!summary) return fieldName === "total_absence_days" ? 0 : 0.0;
    return summary[fieldName] || (fieldName === "total_absence_days" ? 0 : 0.0);
  }
...
class SummaryFieldEditDialog extends Component {
  ...
  onSave() {
    if (this.validateValue()) {
      const finalValue =
        this.props.fieldName === "total_absence_days"
