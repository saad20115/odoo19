...
class TimeoffController(http.Controller):
    ...
    def create_timeoff_request(self, **kwargs):
        ...
        try:
            ...
            li2 = []
            for user in timeoff_request.validation_status_ids:
                li2.append(user.user_id.id)
            for user in timeoff_request.holiday_status_id.validator_ids.filtered(
    ...
    def get_all_pending_requests(self, **kwargs):
        """Get pending time-off requests that current user can approve"""
        try:
            user_id = request.uid
            user_obj = request.env['res.users'].browse(user_id)
            ...
            if not employee:
                return valid_response({"error": "No employee associated with this user"}, status=404)
            ...
            # Build domain to get only requests the current user can approve
            domain = [
                ('state', 'in', ['confirm', 'validate1']),  # Pending states
                '|',  # OR condition
                # Condition 1: User is a full admin for this leave type
                ('holiday_status_id.full_admin_ids', 'in', [user_id]),
                # Condition 2: User is in the validators list and hasn't approved yet
                '&',
            ...
            requests_data = []
            for req in pending_requests:
                # Check approval type for this request
                ...
                # Get validation status for current user
                ...
                vals = {
                    ...
                    "state_display": dict(req._fields['state'].selection)[req.state],
                    "create_date": req.create_date.strftime('%Y-%m-%d %H:%M:%S') if req.create_date else None,
                    # Additional info about approval permissions
                    "can_full_approve": is_full_admin,  # Can approve directly as admin
                    "is_validator": is_validator,  # Is in validators list
                    ...
                    # Show how many validators approved/pending
