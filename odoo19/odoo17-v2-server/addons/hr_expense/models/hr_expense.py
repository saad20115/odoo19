...
class SectionExpense(models.Model):
    _name = "section.expense"
    _description = "Section Model"


class Request(models.Model):
    _name = "request.model"
    _description = "Request Model"

class Exchange(models.Model):
    _name = "exchange.model"
    _description = "Exchange Model"



class Management(models.Model):
    _name = "management.model"
    _description = "Management Model"
...
class HrExpense(models.Model):
    ...
    y_section = fields.Many2one('section.expense', string = "القسم")
    x_request_type = fields.Many2one('request.model', string = "نوع الطلب")
    x_exchange_method = fields.Many2one('exchange.model', string = "طريقة الصرف")
    x_management_approval = fields.Many2one('management.model', string = "اعتماد اإلدارة")
    x_state = fields.Many2one('state.model', string = "الحالة")
    ...
    def _get_default_expense_sheet_values(self):
        ...
        expenses_with_amount = self.filtered(lambda expense: not (
            ...
            or not float_round(expense.quantity, precision_rounding=expense.product_uom_id.rounding or 0.01)
        ))
