  ...
  async _autoCaptureAndVerify() {
    ...
    try {
      const response = await this.rpc("/hr_attendance/verify_employee_face", {
        employee_id: this.props.parent.employee_id,
        image_data: this.img_binary,
        checkout: this.props.parent.checkout,
      });

      if (response.matched) {
        alert(
          this.props.parent.checkout
            ? "✅ Photo Taken! Marking Checkout..."
            : "✅ Photo Taken! Marking Attendance..."
        );
...
CameraDialog.template = "hr_attendance.camera_dialog_attendance";
