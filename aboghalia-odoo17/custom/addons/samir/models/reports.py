...
class EmployeeRequest(models.Model):
    ...
    company_id = fields.Many2one('res.company', string="اسم العميل/الجهة", default=lambda self: self.env.company)
    description = fields.Html(string="الوصف")
    ...
    ('overdue', 'متأخر'),
    ('done', 'منتهى'),
    ], string="Status", compute='_compute_status', store=False, tracking=True)

    employee_ids = fields.Many2many('hr.employee', string="الموظف المسؤول", tracking=True)
    start_date = fields.Date(string="تاريخ البداية" , tracking=True)
    ...
    department= fields.Char(string="القسم" ,  tracking=True)
    incoming_number = fields.Char(string="رقم الوارد")
    created_by = fields.Many2one('res.users', string="Created By", default=lambda self: self.env.user, readonly=True)
    tag_ids = fields.Many2many('employee.request.tag', string="Tags")  # new m2m field for tags
