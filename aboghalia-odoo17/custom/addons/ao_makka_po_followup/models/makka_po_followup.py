...
class PoFollowup(models.Model):
    _name = 'makka.po.followup'
    _description = 'PO Follow-up Makka'
    ...
    office_id = fields.Many2one(
        ...
        tracking=True,
        ondelete='restrict',
        help='Area / office as in Excel الجهة (مكه، خليص، المشاعر).',
    )
    ...
    description = fields.Char(string='وصف العمل', tracking=True)
    execution_date = fields.Date(
        string='تاريخ التنفيذ',
        tracking=True,
    )
    entry_date = fields.Date(
        string='تاريخ الادخال',
        tracking=True,
    )
    ...
    invoice_number = fields.Char(string='رقم الفاتورة', tracking=True)
    invoice_date = fields.Date(string='تاريخ الفوترة', tracking=True)
    estimation_date = fields.Date(string='Estimation Date', tracking=True)
    uploaded_to_system = fields.Selection(
        YES_NO_SELECTION,
        string='رفع على النظام',
        default='no',
        tracking=True,
    )
    disbursement = fields.Selection(
        ...
        default='no',
        tracking=True,
        help='Disbursement status as in the Excel sheet (تم / لم يتم).',
    )
    expenditure = fields.Float(
        string='Expenditure Amount',
        digits=(16, 2),
        tracking=True,
        help='Monetary expenditure amount (kept for compatibility; Excel الصرف is yes/no).',
    )
    value = fields.Float(
        string='القيمة',
        digits=(16, 2),
    ...
    def action_assign_to_accounting(self):
        ...
        payload = self.env['makka.po.accounting.api'].create_draft_invoices(items)
        results = {str(row.get('followup_id')): row for row in (payload.get('results') or [])}

        success = self.env['makka.po.followup']
        errors = []
        now = fields.Datetime.now()
        for record in records:
            row = results.get(str(record.id)) or {}
            if row.get('ok'):
                invoice_name = self.env['makka.po.order']._clean_invoice_name(
    ...
    def mark_sent_to_portal(self, invoice_name=None, invoice_id=None):
        ...
        cleaned = self.env['makka.po.order']._clean_invoice_name(invoice_name)
        for record in self:
            ...
            record.write(vals)
            record.message_post(body=_(
                'Accounting sent this PO to Portal Makka PO Order (invoice: %s).'
    ...
    @api.model
    def action_open_import_fix_wizard(self):
        return {
            'type': 'ir.actions.act_window',
            'name': _('Fix Import File'),
            'res_model': 'makka.po.import.fix.wizard',
            'view_mode': 'form',
            'target': 'new',
        }
