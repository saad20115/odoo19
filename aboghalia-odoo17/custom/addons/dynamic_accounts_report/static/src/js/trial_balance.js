...
const luxonObj = luxon;
luxonObj.Settings.defaultLocale = "en-US";
luxonObj.Settings.defaultNumberingSystem = "latn";
const today = luxonObj.DateTime.now();
let monthNamesShort = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]

class TrialBalance extends Component {
    async setup() {
        super.setup(...arguments);
        // console.log(luxon.Settings);
        // console.log(today.startOf('year').minus({ days: 1 }).startOf('year').toFormat('yyyy-MM-dd'));
        ...
        this.state = useState({
            ...
            searched_account_list: null,
            searched_account_list_top_bar: null,
        ...
        await this.load_data(self.initial_render = true);
        await this.state.data.sort((a, b) => a.account.localeCompare(b.account));
        await this.setAccountGroups();
    ...
    async load_data() {
        ...
        try {
            ...
            self.start_date.el.value = startOfMonth.getFullYear() + '-01-01';

            self.end_date.el.value = endOfMonth.getFullYear() + '-12-31';

            // self.state.date_viewed.push(monthNamesShort[today.getMonth()] + '  ' + today.getFullYear())
            self.state.date_viewed.push(today.getFullYear())
            $.each(self.state.data, function (index, value) {
                self.state.journals = value.journal_ids
                self.state.accounts = value.account_ids
                self.state.groups = value.group_ids
    ...
    async setAccountGroups(){
        var groupsRet = await this.state.groups;
        groupsRet = JSON.parse(JSON.stringify(groupsRet));
        if(groupsRet){
            for(var group in groupsRet){
                // console.log(groupsRet[group]);
                if(groupsRet[group].code_prefix_start == groupsRet[group].code_prefix_end){
                    this.state.groups_list.push({name:groupsRet[group].name,range:[groupsRet[group].code_prefix_start]});
                    continue;
                }
                else{
                    var current = Number(groupsRet[group].code_prefix_start);
                    var rangeList = []
                    while(current <= Number(groupsRet[group].code_prefix_end)){
                        rangeList.push(current+'');
                        current++;
                    }
                    this.state.groups_list.push({name:groupsRet[group].name,range:rangeList});
                }
        ...
        // console.log(this.state.groups_list[0].name);
    }

    showGroupsTable(){
        var checkGroup = document.getElementById("flexCheckDefault");
        var tableGroup = document.getElementById("tableGroup");
        var tableMain = document.getElementById("tableMain");
        if(checkGroup.checked){
            tableMain.style.display = 'none';
            tableGroup.style.display = 'table';
        }
        else{
    ...
    async applyFilter(val, ev, is_delete) {
        ...
        if (ev) {
            ...
            if (val && val.target.name === 'start_date') {
                ...
                // console.log(this.start_date.el.value);
                // console.log(this.end_date.el.value);
                this.state.date_viewed = []
    ...
    let startYear, startMonth, startDay, endYear, endMonth, endDay;
        ...
        const selectedAccountNames = selectedAccountIDs.map((accountID) => {
          const account = self.state.accounts[accountID];
          return account ? account.name : ''; // Return the name if account exists, otherwise an empty string
        });
        let filters = {
            'journal': selectedJournalNames,
            'account_acc': selectedAccountNames,
    ...
    */
    ...
    SearchBarAccounts(ev) {
        if(this.state.accounts){
            this.state.searched_account_list = this.state.accounts.filter(function (el) {
                return el.name.includes(ev.target.value);
              });
        }
        // console.log(ev.target.value);
    }
    SearchBarAccountsTop(ev) {
        if(this.state.accounts){
            this.state.searched_account_list_top_bar = this.state.data.filter(function (el) {
                return el.account.includes(ev.target.value);
              });
        }
    }
...
registry.category("actions").add("trl_b", TrialBalance);
