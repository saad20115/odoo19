...
class HrAttendanceEditRequest(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', string="Employee", required=True, default=lambda self: self.env.user.employee_id.id , tracking=True)
    attendance_id = fields.Many2one('hr.attendance', string="Attendance", required=True, domain="[('employee_id', '=', employee_id)]")
    date = fields.Datetime(related='attendance_id.check_in', string="Attendance Date", store=True, tracking=True)

    check_in_new = fields.Datetime(string="New Check In", tracking=True)
    check_out_new = fields.Datetime(string="New Check Out", tracking=True)
    ...
    approved_by = fields.Many2one('hr.employee', string="Approved By", readonly=True, copy=False, tracking=True)
