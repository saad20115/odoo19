...
class CustomerQrTemplate(models.AbstractModel):
    ...
    _name = 'report.equipment_fleet_qrcode.customer_qr_template'

    @api.model
    def _get_report_values(self, docids, data=None):
        """Get the report values for generating the customer QR template."""
        if data['type'] == 'cust':
            dat = [request.env['fleet.vehicle'].browse(data['data'])]
        elif data['type'] == 'all':
            dat = [request.env['product.product'].search(
                [('product_tmpl_id', '=', data['data'])])]
        else:
            dat = request.env['maintenance.equipment'].browse(data['data'])
        return {
            'data': dat,
        }
